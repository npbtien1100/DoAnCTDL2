...
//khoi tao ban tan so cho cac ki tu
...
//tao ma huffman cho cac ki tu co tren cay huffman
...
//ham chuyen doi 1 chuoi bit sang so he 10
...
//chuyen so he 10 sang chuoi bit
...
//hàm dựng cây huffman từ kí tự và mã huffman của nó
...
{//build a new branch according to the inpue code and ignore the already existed branches
	//nếu duyệt tới node lá thì tạo node mới
	...
	//gán kí tự cho node sau khi đã duyệt qua đường đi đến node đó
...
//tạo hàng đợi ưu tiên từ bảng tần số của các kí tự
void huffman::create_pq()
{
	in_file.open(in_file_name, ios::in);
	// duyệt qua tất cả kí tự có trong file và tăng tần số của kí tự đó
	...
	in_file.close();
	//đưa các kí tự có tần số khác 0 vào hàng đợi ưu tiên
	for (int i = 0; i < 256; i++)
	{
...
//tạo cây huffman từ hàng đợi ưu tiên
void huffman::create_huffman_tree()
{
	priority_queue<node_ptr, vector<node_ptr>, compare> temp(pq);
	while (temp.size() > 1)
	{//create the huffman tree with lowest frequecy characher being leaf from bottom to top
		//tạo cây huffman với kí tự có tần số nhỏ nhất là lá
...
//tạo ra mã huffman cho tất cả kí tự có trên cây huffman 
...
void huffman::encode()
{
	...
	// xuat ma huffman cua cac ki tu
	/*for (int i = 0; i < 256; i++)
	{
		if (node_array[i]->freq)
			cout << "Ki tu: " << node_array[i]->id << "   ma huffman: " << node_array[i]->code << endl;
	}*/
	this->coding_save();
	cout << "Finish!" << endl;
}
